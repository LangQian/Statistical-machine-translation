  ...
  for l=1:length(lines)
    ...
    for index=1:length(words)
        if isfield(LM.uni,words{index})
            LM.uni.(words{index}) = LM.uni.(words{index}) + 1;
        else 
            LM.uni.(words{index}) = 1;
        end
        if index<=(length(words)-1)
            if isfield(LM.bi, words{index})
                if isfield(LM.bi.(words{index}), words{index+1})
                    LM.bi.(words{index}).(words{index+1}) = LM.bi.(words{index}).(words{index+1}) + 1;
                else 
                    LM.bi.(words{index}).(words{index+1}) = 1;
                end
            else
                LM.bi.(words{index}).(words{index+1}) = 1;
            end
